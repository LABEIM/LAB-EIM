...
3. **`comment-pr` (Post PR Preview Comment)**: Posts a formatted table with live Cloudflare and Vercel preview deployment URLs, commit SHA, and workflow run link back to the Pull Request.
4. **`lighthouse`**: Audits performance with **3 runs** (median score) against the live deployed Cloudflare URL. On **Pull Requests**, a failing Lighthouse score marks the PR with a failed status check, preventing merge to `main` (the preview deployment itself is harmless). On **push to main**, Lighthouse serves as post-deploy monitoring — failures generate alerts but the code is already live. Score thresholds:
